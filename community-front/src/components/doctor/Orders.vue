...
export default {
  ...
  methods: {
    //同意请求
    agree(index, row) {
      this.order.status = 1;  //同意预约，状态置为1
      ...
      this.order.orderTime = row.orderTime;
      this.msg = '已同意预约!';
      this.flag=false;
      this.updateOrder();
      this.flag=true;
    ...
    reject(index, row) {
      ...
      this.order.status = -1;  //拒绝预约，状态置为1
      this.order.username = row.username;
      this.order.doctorName = row.doctorName;
      this.order.dName = row.dName;
      this.order.uName = row.uName;
    ...
    getOrder() {
      axios({
        method: 'get',
        url: '/order/selectByDoctorName?doctorName=' + this.$store.state.login.id
      }).then(resp => {
